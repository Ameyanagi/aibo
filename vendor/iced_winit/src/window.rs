...
/// The color behind the IME preedit overlay.
///
/// [AIBO PATCH] Upstream forces the window's `background_color` to `a: 1.0`,
/// which turns the strip **opaque black** on a transparent window — invisible
/// composing text on a dark theme, and black-on-black-adjacent unreadability
/// on a light one (owner report, 2026-08-03). A transparent window gives the
/// overlay nothing honest to inherit, so derive an opposing backdrop from the
/// text color instead: light text gets a dark strip, dark text a light one.
fn preedit_backdrop(background: Color, text: Color) -> Color {
    if background.a > 0.0 {
        return Color {
            a: 1.0,
            ..background
    ...
    let luma = 0.299 * text.r + 0.587 * text.g + 0.114 * text.b;
    if luma > 0.5 {
